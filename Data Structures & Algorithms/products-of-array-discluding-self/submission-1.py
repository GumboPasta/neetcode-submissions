class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        # Create empty arrays
        prefix = [0] * len(nums)    # Will store products of all elements to the left of i
        postfix = [0] * len(nums)   # Will store products of all elements to the right of i
        ...
        for i, num in enumerate(nums):
            ...
            # Assign the left product
            prefix[i] = left 

            # Assign the last position in the array and assign the right product
            j = -i - 1
            postfix[j] = right

            # Update next products with current positions's number
            left *= nums[i]
            right *= nums[j]
