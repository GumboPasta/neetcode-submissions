class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        if nums[0] < nums[n - 1]:                                # case 1: if array is in same position -> normal binary search
            l, r = 0, n - 1
        elif target > nums[n - 1] and target <= nums[min_index - 1]: # case 2: if array is in left half -> meaning it need to be greater than the last number and less than the mid point
            l, r = 0, min_index - 1
        else: 
            l, r = min_index, n - 1
